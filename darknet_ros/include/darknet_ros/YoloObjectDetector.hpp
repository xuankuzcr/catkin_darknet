...
{
public:
  YoloV3BoxCxn(float _x, float _y, float _w, float _h)
  {
    this->x = _x;
    this->y = _y;
    this->w = _w;
  ...
  float x, y, w, h;
  int classId;
  bool Inside(const YoloV3BoxCxn &other)
  {
    return (
        (this->x >= other.x) &&
        (this->y >= other.y) &&
...
struct YoloV3BoxCxnCmp
{
  bool operator()(const YoloV3BoxCxn &a, const YoloV3BoxCxn &b)
  {
...
private:
  ...
  cv::Mat pnpMat_;
  cv::Mat yoloMat_;
  boost::shared_mutex mutexpnpMat_;
  std::priority_queue<YoloV3BoxCxn, std::vector<YoloV3BoxCxn>, YoloV3BoxCxnCmp> aimArmorQueue[2];
  void ipp(cv::Mat &InputImage, cv::Mat &OutputImage);
  void SolvePnPCxn(cv::Rect &rect);
};
